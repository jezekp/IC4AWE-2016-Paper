...
\title{MoBio\subtitle{A mobile system for collecting electrophysiological data} }
...
With regard to the use of open source technologies in maximal range we decided to implement system for Android platform. The main benefit of this selection is its availability on large collections of devices including both tablets and mobile phones. Moreover, there is lot of cheap devices on the market. 
...
When data are collected they are stored on the device SD card. Although data are visualized directly on mobile phone screen they are simultaneously transfered to the server. Data on the server are stored for future processing.

Technically EEGBase provide a RESTful API with methods for obtaining user's experiments and uploading newly collected data. The user can select an automatic data upload triggered when the user is on-line or he/she can enforce upload manually.
