...
\noindent 
We can observe that there is a diverse collection of available sensors on the market. These sensors vary in used data format, topology or transfer protocol. When the data are processed they must be collected and stored in repositories. We identify topologies distinguishing sensors and selected the most suitable for obtaining and storing the data. Because of producers of sensors uses own systems and proprietary data formats we presented a prototype of own terminology used for expressing data from sensors. This terminology is implemented in an Android system. A limited set of sensors is supported in this approach. A complete process from collecting data by the mobile system, its storage in a universal format respecting presented ontology, and their storage in EEGBase is outlined. This innovative approach can serve as a standardized description and way of transfer data from sensors to remote databases. Respecting ANT+ or Bluetooth profiles by sensors producers is only one limited factor.
