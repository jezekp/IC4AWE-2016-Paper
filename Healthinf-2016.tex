...
In this paper we are proposing a terminology describing data originating from health sensors and a prototype of mobile client for collecting data from these sensors. This client provides integration with a limited set of devices and enables data to be transferred and visualized in a remote storage.
...
\begin{itemize}
 ...
 \item two-layers - The sensor only measures the data that sens to a remote system. This system can be a common computer/laptop or a smart phone/watch etc.
...
There are a lot of sensors in one-layer topology including various glucometers, tonometers, stepcounters etc. These devices usually enable easy manipulation and do not require connection to any other system. Anyway, the data are stored in a proprietary storage that do not provide means for communication with other systems. Two layers topology includes sensors such as heart rate belts or some step counters. They do not have own display, but they usually use technologies Bluetooth or ANT (resp ANT+)\footnote{ANT is the protocol while ANT+ is a set of mutually agreed upon definitions for what the information sent over ANT represents.} for the data transfer. Data are usually displayed on a smart device screen. Because of limited performance of mobile devices only a basic data processing is provided. The advanced data processing is performed on a remote server that is added in the three layers topology. There are few exceptions as BlibCare tonometer equipped by WiFi. It enables a direct data transfer to a server using a home network.
